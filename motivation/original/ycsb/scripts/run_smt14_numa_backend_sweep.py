...
# TARGET_CPUS = (0, 84, 1, 85, 2, 86, 3, 87, 4, 88, 5, 89, 6, 90)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fixed-workload backends on a 14-CPU SMT-aware scaling sequence without external noise."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the resolved assignments and temp config plan without running the harness.",
    )
    return parser.parse_args()


def derived_result_prefix(base_result_prefix: str, workload_numa: int, count: int) -> str:
    stem = f"{base_result_prefix.removesuffix('_fixed_workload_noise')}_smt14"
...
def build_temp_config(base_config_path: Path, count: int, workload_numa: int) -> tuple[str, dict]:
    ...
    selected_cpus = list(TARGET_CPUS[:count])
...
def run_one(base_config_path: Path, count: int, workload_numa: int, dry_run: bool) -> None:
    ...
    selected_cpus = temp_config["assignments"][0]["cores"]
    print(
        f"plan config={base_config_path.name} numa={workload_numa} count={count} "
        f"cpus={selected_cpus} result_prefix={result_prefix} df=CCM[0]"
    )
    if dry_run:
        return
    ...
    with tempfile.TemporaryDirectory(
        dir=ROOT_DIR,
        prefix=f"{base_config_path.stem}.smt14.numa{workload_numa}.x{count}.",
    ) as temp_dir_raw:
        temp_dir = Path(temp_dir_raw)
        config_path = temp_dir / f"{base_config_path.stem}.smt14.numa{workload_numa}.x{count}.json"
