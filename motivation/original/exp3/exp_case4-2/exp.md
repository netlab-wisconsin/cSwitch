...
This experiment now uses `ccd0`, `ccd1`, `ccd3`, and `ccd4` from the first
socket, skipping `ccd2` because it lacks CCM mapping in the current host setup.

Using 4 chiplets (0-27). 
...
3. policy notes
* The harness supports the legacy `eevdf` taskset-only baseline and pinned-placement policies (`optimal`, `optimal-pinned`, `same-chiplet`).
