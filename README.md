...
Scheduler builds use a per-account scratch directory at
`/dev/shm/scx-ae-build-<uid>` so files from another account cannot block
cleanup. Set `AE_BUILD_SCRATCH_ROOT` only when a different local scratch
