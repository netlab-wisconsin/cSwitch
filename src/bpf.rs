...
pub struct QueuedTask {
    pub tid: i32,             // tid that uniquely identifies a task
    pub tgid: i32,            // thread-group id of the task
    pub current_cpu: i32,     // CPU previously used by the task
    pub current_domain: i32,  // LLC/domain currently associated to the task
    ...
    pub last_fill_bw_mib_s_x100: [u32; crate::types::MEM_SOURCE_COUNT],
    pub ewma_fill_bw_mib_s_x100: [u32; crate::types::MEM_SOURCE_COUNT],
...
impl EnqueuedMessage {
    ...
    fn to_queued_task(&self) -> QueuedTask {
        QueuedTask {
            ...
            current_cpu: self.inner.current_cpu,
            current_domain: self.inner.current_domain,
            ...
            last_l2_bw_mib_s_x100: self.inner.last_l2_bw_mib_s_x100,
            ewma_l2_bw_mib_s_x100: self.inner.ewma_l2_bw_mib_s_x100,
            last_fill_bw_mib_s_x100: self.inner.last_fill_bw_mib_s_x100,
            ewma_fill_bw_mib_s_x100: self.inner.ewma_fill_bw_mib_s_x100,
            last_ipc_x1000: self.inner.last_ipc_x1000,
            ewma_ipc_x1000: self.inner.ewma_ipc_x1000,
            last_stall_pct_x100: self.inner.last_stall_pct_x100,
            ewma_stall_pct_x100: self.inner.ewma_stall_pct_x100,
            trigger: self.inner.trigger,
            tick_seq: self.inner.tick_seq,
            last_update_ns: self.inner.last_update_ns,
...
impl<'cb> BpfScheduler<'cb> {
    pub fn init(
        ...
        builtin_idle: bool,
        slice_ns: u64,
        tick_reeval_every: u32,
        l2_need_mib_s_x100: u32,
        tick_defer_max: u32,
        llc_stale_ns: u64,
        df_stale_ns: u64,
        name: &str,
        ...
        skel.maps.rodata_data.as_mut().unwrap().enable_tick_resched =
            cfg!(feature = "tick-resched");
        skel.maps.rodata_data.as_mut().unwrap().enable_light_compete =
            cfg!(feature = "light-compete");
        skel.maps.rodata_data.as_mut().unwrap().enable_bpf_stay_fastpath =
            cfg!(feature = "bpf-stay-fastpath");
        skel.maps.rodata_data.as_mut().unwrap().tick_reeval_every = tick_reeval_every;
        skel.maps.rodata_data.as_mut().unwrap().l2_need_mib_s_x100 = l2_need_mib_s_x100;
        skel.maps.rodata_data.as_mut().unwrap().tick_defer_max = tick_defer_max;
        skel.maps.rodata_data.as_mut().unwrap().llc_stale_ns = llc_stale_ns;
        skel.maps.rodata_data.as_mut().unwrap().df_stale_ns = df_stale_ns;
        skel.maps.rodata_data.as_mut().unwrap().helper_slice_ns = slice_ns / 8;
        ...
        // The main userspace scheduler thread must be a sched_ext task so the
        // BPF dispatcher can run it from SCHED_DSQ and then drain the user
        // ring buffer on the next dispatch pass.
        if partial {
            Self::enter_sched_ext_current_thread()?;
        }
    ...
    // Publish how many tasks the user-space scheduler still has queued locally.
    pub fn publish_scheduled(&mut self, nr_pending: u64) {
    ...
    pub fn notify_complete(&mut self, nr_pending: u64) {
        self.publish_scheduled(nr_pending);
    ...
    // Set scheduling class for the current scheduler thread to SCHED_EXT.
    pub fn enter_sched_ext_current_thread() -> Result<()> {
    ...
    fn use_sched_ext() -> i32 {
        ...
        unsafe { libc::sched_setscheduler(0, SCHED_EXT, &param as *const sched_param) }
    }

    // Pick an idle CPU for the target PID.
    #[allow(dead_code)]
    pub fn select_cpu(&mut self, tid: i32, cpu: i32, flags: u64) -> i32 {
    ...
    pub fn dispatch_task(&mut self, task: &DispatchedTask) -> Result<(), libbpf_rs::Error> {
        ...
        *vtime = task.vtime;
        *enq_cnt = task.enq_cnt;
        *tick_holdoff = task.tick_holdoff;
